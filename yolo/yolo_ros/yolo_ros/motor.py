...
class Humanfollower(Node):
    def __init__(self):
        ...
        # self.declare_parameter('kp_ang',0.3)  # 회전 비례계수 (rad/s)
        # self.declare_parameter('kp_lin', 0.5)  # 기본값 0.5 (원하는 값으로 설정)
        # self.declare_parameter('max_bwd', -0.2)  # 후진 시 최대 속도. 단위: m/s
        # self.declare_parameter('max_fwd', 0.2)  # 전진 시 최대 속도. 단위: m/s
        ...
        # Mode publisher
        self.vel_pub = self.create_publisher(UInt8,'cmd_mode', qos_profile)
    ...
    def cmd_based_dis(self, msg: Detection2DArray):
        ...
        for det in msg.detections:
            ...
            # 거리 추정 -------------------------------------------------------
            label = det.results[0].hypothesis.class_id  # "Person1" 같은 문자열
            ...
            # # 선속도(v) 계산 --------------------------------------------------
            # v = self.get_parameter('kp_lin').value * err_d      # 비례제어
            # v = max(self.get_parameter('max_bwd').value,
            #         min(self.get_parameter('max_fwd').value, v))
            ...
            #omega = self.get_parameter('kp_ang').value * direction
            # # ── /cmd_vel 퍼블리시 -----------------------------------
            # cmd = Twist()
            # cmd.linear.x  = 0.0          # 여기선 회전만 제어
            # cmd.angular.z = omega
